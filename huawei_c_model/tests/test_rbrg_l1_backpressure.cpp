...
TEST_F(Phase4Test, RBRGL1Backpressure) {
    ...
    // Queue depth = 1 (ingress) + pipeline(2) + egress (1) = 4 total capacity
    // 4 flits injected. None injected to ringB because it's blocked.
    // So all 4 flits should be stuck in the bridge.

    EXPECT_EQ(bridge.egress_queue.size(), 1);
    EXPECT_EQ(bridge.ingress_queue.size(), 1);
