...
*   **Cycle 8 ~ 11 (背壓 Backpressure 發生)**：
    ...
    垂直環上晚到的 F104 與 F105 抵達交會點後，同樣被 Bridge 吸收，並在 Ingress 佇列或內部管線裡排隊等待（此階段在畫面上不可見，屬於隱形的內部狀態），依序等待 Bridge 處理。它們並沒有被偏折回環上。
...
3. 最重要的是，精準模擬出了遇到熱點時的 **背壓 (Backpressure)** 排隊與內部管線物理現象！
